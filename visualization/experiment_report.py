...
def evaluate_checkpoint(ckpt_path, data_root, envs, dim, seq_len=20,
    ...
    """
    修复：
    1. DataLoader 返回三元组，正确解包（原版 for x, y in loader 会报错）。
    2. dim 默认值统一由调用方传入，不在函数内硬编码。
    3. load_state_dict 加 strict=False，兼容新旧结构。
    """
    ...
    all_pred, all_gt = [], []
    # 修复：三元组解包，第三项 root_offset 不参与评估
    for x, y, _ in loader:
        x    = x.to(device)
        pred, *_ = model(x, x, alpha=0.0)
        all_pred.append(pred.cpu())
        all_gt.append(y[:, 0].cpu())   # 取第 0 帧作为 GT（与训练对应）
...
def plot_comprehensive_comparison(results, save_path):
    ...
    fig.suptitle("Experiment Comparison Report", fontsize=15,
...
def main():
    ...
    if args.labels is None:
        args.labels = [
            os.path.basename(os.path.dirname(c)) for c in args.checkpoints
        ]
    ...
    for metric in ["MPJPE", "PA-MPJPE", "PCK@0.05"]:
        plot_metric_bars(
            ...
            os.path.join(
                args.save_dir,
                f"compare_{metric.replace('@', '_').replace('-', '_')}.png",
            ),
            ylabel=metric,
        )
