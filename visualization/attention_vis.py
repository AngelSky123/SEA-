...
def extract_attention_weights(model, x, device):
    ...
    返回 dict: {'sensor': (B, N, N)}

    修复：
    1. 不再手动复现 encoder 调用链（原代码引用了不存在的 gat.fc 属性）。
    2. 改为直接调用完整的 model.encoder(x)，触发 CrossSensorAttention
       的 forward，从而将 last_attn_weights 写入缓存。
    3. DataLoader 返回三元组，正确解包。
    """
    model.eval()
    x = x.to(device)
    attn_weights = {}
    ...
    with torch.no_grad():
        # 直接走完整 encoder 前向，触发 CrossSensorAttention 的缓存写入
        _ = model.encoder(x)   # (B, T, N, D)

        w = model.encoder.attn.last_attn_weights   # (B*T, N, N) 或 None
        if w is not None:
            B, T = x.shape[0], x.shape[1]
            N    = w.shape[-1]
            # 在 T 维度取平均：(B*T, N, N) → (B, N, N)
            w_avg = w.view(B, T, N, N).mean(dim=1)
            attn_weights["sensor"] = w_avg.cpu().numpy()
...
def main():
    ...
    # 修复：DataLoader 返回三元组，正确解包
    for i, (x, _, __) in enumerate(loader):
        if i >= args.n_batches:
            break
